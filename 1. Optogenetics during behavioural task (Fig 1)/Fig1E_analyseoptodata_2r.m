function [b, goodses, PS,includedMice] = ...
    analyseoptodata_2r(loc, bootstrapAll, nB, plotit);
...
for m = 1:mousetot
    ...
    if sum(basetrialsWopto) >2
        [behaviourTemp] = getoptoresults_2r(dat,basetrialsWopto,mn,'Base task',plotit);
        ...
        for i=1:length(b.(mn).baseOptoH)-1
            ...
            if b.(mn).baseOptoP(i) < b.(mn).baseOptoP(end)
                b.(mn).basePperf(i) = binocdf(b.(mn).baseOptoH(i), b.(mn).baseOptoH(i)+ ...
                    b.(mn).baseOptoE(i), b.(mn).baseOptoP(end));
            else
                b.(mn).basePperf(i) = binocdf(b.(mn).baseOptoH(i),b.(mn).baseOptoH(i)+ ...
                    b.(mn).baseOptoE(i),b.(mn).baseOptoP(end), 'upper') +  ...
            ...
            b.(mn).basePchance(i) = binocdf(b.(mn).baseOptoH(i), b.(mn). baseOptoH(i)+ ...
                b.(mn).baseOptoE(i), 0.5, 'upper') + ...
                binopdf(b.(mn).baseOptoH(i), b.(mn). baseOptoH(i)+ ...
                b.(mn).baseOptoE(i), 0.5);
        end
    ...
    if sum(fulltrialsWoptoOri) >2
        [behaviourTemp] = getoptoresults_2r(dat,fulltrialsWoptoOri,mn,'Full Ori',plotit);
        ...
        for i=1:length(b.(mn).fullOptoHOri)-1
            ...
            if b.(mn).fullOptoPOri(i) < b.(mn).fullOptoPOri(end)
                b.(mn).fullPperfOri(i) = binocdf(b.(mn).fullOptoHOri(i), b.(mn).fullOptoHOri(i)+ ...
                    b.(mn).fullOptoEOri(i), b.(mn).fullOptoPOri(end));
            else
                b.(mn).fullPperfOri(i) = binocdf(b.(mn).fullOptoHOri(i),b.(mn).fullOptoHOri(i)+ ...
                    b.(mn).fullOptoEOri(i),b.(mn).fullOptoPOri(end), 'upper') +  ...
                    binopdf(b.(mn).fullOptoHOri(i),b.(mn).fullOptoHOri(i)+ ...
                    b.(mn).fullOptoEOri(i),b.(mn).fullOptoPOri(end));
            end
            %get p-value for performance vs. chance level
            b.(mn).fullPchanceOri(i) = binocdf(b.(mn).fullOptoHOri(i), b.(mn). fullOptoHOri(i)+ ...
                b.(mn).fullOptoEOri(i), 0.5, 'upper') + ...
                binopdf(b.(mn).fullOptoHOri(i), b.(mn). fullOptoHOri(i)+ ...
                b.(mn).fullOptoEOri(i), 0.5);
        end
        %do bonferroni correction
    ...
    if sum(fulltrialsWoptoOOP) >2
        [behaviourTemp] = getoptoresults_2r(dat,fulltrialsWoptoOOP,mn,'Full OOP',plotit);
        ...
        for i=1:length(b.(mn).fullOptoHOOP)-1
            ...
            if b.(mn).fullOptoPOOP(i) < b.(mn).fullOptoPOOP(end)
                b.(mn).fullPperfOOP(i) = binocdf(b.(mn).fullOptoHOOP(i), b.(mn).fullOptoHOOP(i)+ ...
                    b.(mn).fullOptoEOOP(i), b.(mn).fullOptoPOOP(end));
            else
                b.(mn).fullPperfOOP(i) = binocdf(b.(mn).fullOptoHOOP(i),b.(mn).fullOptoHOOP(i)+ ...
                    b.(mn).fullOptoEOOP(i),b.(mn).fullOptoPOOP(end), 'upper') +  ...
                    binopdf(b.(mn).fullOptoHOOP(i),b.(mn).fullOptoHOOP(i)+ ...
                    b.(mn).fullOptoEOOP(i),b.(mn).fullOptoPOOP(end));
            end
            %get p-value for performance vs. chance level
            b.(mn).fullPchanceOOP(i) = binocdf(b.(mn).fullOptoHOOP(i), b.(mn). fullOptoHOOP(i)+ ...
                b.(mn).fullOptoEOOP(i), 0.5, 'upper') + ...
                binopdf(b.(mn).fullOptoHOOP(i), b.(mn). fullOptoHOOP(i)+ ...
                b.(mn).fullOptoEOOP(i), 0.5);
        end
        %do bonferroni correction
        ...
        if stim == 1 %base task
            if isfield(dat, 'baseOptoLats')
                ...
                for LL = 1:length(useLats)
                    ...
                    NumTrsTot(m,stim,latIdx) = dat.baseOptoH(LL) + dat.baseOptoE(LL);
                end
            else
                continue
            end
        elseif stim == 2 %Ori
            if isfield(dat, 'fullOptoLatsOri')
                ...
                for LL = 1:length(useLats)
                    ...
                    %Number of trials at each entry of 'StimLevels'
                    NumTrsTot(m,stim,latIdx) = dat.fullOptoHOri(LL) + dat.fullOptoEOri(LL);
                end
            else
                continue
            end
        elseif stim == 3 %OOP
            if isfield(dat, 'fullOptoLatsOOP')
                ...
                for LL = 1:length(useLats)
                    ...
                    %Number of trials at each entry of 'StimLevels'
                    NumTrsTot(m,stim,latIdx) = dat.fullOptoHOOP(LL) + dat.fullOptoEOOP(LL);
                end
            else
                continue
            end
...
%             avg_perf = nansum((tempPerf.*temptrCount)./sum(temptrCount),1);
        ...
        xlim([0 270])
        title(['Stim ' num2str(stim) ', n=' num2str(max(nanCount))]);
...
%         end
        ...
        EB = errorbar(infPoint(stim), 0.47, std_thres(stim),  'horizontal' );
