...
for rf = rfSel
    ...
    for task =1:3
        ...
        prepData{rf,task} = nan(2,size(orgData,2),length(px));
        ...
        for fg = 1:2 %figure vs. ground
            ...
            for uidx = 1:size(orgData,2)
                ...
                prepData{rf,task}(fg,uidx,:) = nanmean(catData,1); %avg within cell and store
            end
            
            %average across cells and smooth the lines
            plotData_tosm(fg,:) = squeeze(nanmean(prepData{rf,task}(fg,:,:),2));
            plotData(fg,:) = smooth(plotData_tosm(fg,:),smfvis);
            plotDataSem_tosm(fg,:) = squeeze(nansem_large(squeeze(prepData{rf,task}(fg,:,:)),1));
        ...
        % get n
        get_cell_ids{rf,task} = whichCellSel(find(~isnan(nanmean(mean(prepData{rf,task},1),3))));
...
%         end
        ...
        nPx = px(ixPx);
        statDataFig = squeeze(prepData{rf,task}(1,:,ixPx))';
        statDataGnd = squeeze(prepData{rf,task}(2,:,ixPx))';
...
%         
    ...
    for uidx = 1:size(prepData{rf,task},2)
        if allRFs(whichCellSel(uidx)) ~= rf 
            continue
        end
